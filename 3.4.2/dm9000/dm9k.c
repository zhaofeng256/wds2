...
static const struct net_device_ops dm9k_netdev_ops = {
	...
	//.ndo_set_features	= dm9k_set_features,
...
int __init dmfe_probe1(struct net_device *dev)
{
	...
	/* bank4 bus width 16bit */
	bwscon = (u32 *)ioremap(0x48000000, 4);
	*bwscon &= ~(0x3 << 16);
	*bwscon |= (0x1 << 16);

	/* Tacc 20ns Tcoh 10ns */
	bankcon4 = ioremap(0x48000014, 4);
	*bankcon4 &= ~(0x7 << 8| 0x3 << 6);
	//*bankcon4 |= 0x1 << 8| 0x1 << 6;
	*bankcon4 |= 0x7 << 8| 0x1 << 6;
	...
	do {
		...
		if (id_val == DM9KS_ID || id_val == DM9010_ID) {
			...
			memset((board_info_t *)netdev_priv(dev), 0, sizeof(struct board_info));
